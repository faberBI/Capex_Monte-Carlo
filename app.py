...
from capex.visuals import (
    ...
    plot_payback_distribution, plot_probs_kri, plot_car_kri, plot_irr_trends,
...
# -----------------------------
def run_simulations(
    df, n_sim, discount_rate, tax_rate,
    ...
    # Lettura colonne con fallback a 0
...
if st.session_state.logged_in:
    ...
    if run_button and uploaded_file is not None:
        ...
        npv_array, fcf_matrix, fcf_pv_matrix, npv_cum_matrix, years_col, costs_fixed, capex = run_simulations(
            df=df, n_sim=int(n_sim), discount_rate=float(discount_rate), tax_rate=float(tax_rate),
            shift_rev_probs=rev_shift_dict, shift_cs_probs=cs_shift_dict,
            shift_capex_probs=capex_shift_dict, shift_disposal_probs=disp_shift_dict
        )
        ...
        st.pyplot(plot_cashflows(fcf_matrix, n_years, project_name))
