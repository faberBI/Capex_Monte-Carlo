...
def parse_beta_grid(raw):
    ...
    for i in range(hr + 1, len(raw)):
        ...
        nn = _norm(name)
        if name is None or str(name).strip() == "" or \
           nn.startswith(("total", "grand total")) or "total market" in nn:
            continue                                   # salta righe di totale
...
# ==========================================================================
# lettura (sceglie il foglio giusto: i file Damodaran hanno più fogli,
# i dati stanno su 'Industry Averages' / il foglio che contiene l'intestazione)
# ==========================================================================
def read_grid(source, header_keyword):
    """Legge il file (path o BytesIO) e restituisce la griglia grezza (header=None)
    del FOGLIO che contiene `header_keyword`. Fallback: primo foglio."""
    sheets = pd.read_excel(source, header=None, engine="xlrd", sheet_name=None)
    for _name, g in sheets.items():
        if _find_header_row(g, header_keyword) is not None:
            return g
    return next(iter(sheets.values()))


def _fetch_grid(url, header_keyword, timeout=60):
    req = Request(url, headers={"User-Agent": "Mozilla/5.0 (DCF tool updater)"})
    with urlopen(req, timeout=timeout) as r:
        data = r.read()
    return read_grid(io.BytesIO(data), header_keyword)
...
def build_dataset(out_path="damodaran_dataset.json", verbose=True):
    ...
    for region, fname in BETA_FILES.items():
        url = BASE + fname
        try:
            raw = _fetch_grid(url, "Industry Name")
    ...
    try:
        erp = parse_erp_grid(_fetch_grid(BASE + ERP_FILE, "Country"))
        if verbose:
    ...
    return _write_dataset(betas, erp,
                          "Aswath Damodaran, Stern NYU (pages.stern.nyu.edu/~adamodar)",
                          out_path, verbose)


def _write_dataset(betas, erp, source, out_path, verbose=True):
    dataset = {
        "metadata": {"source": source, "date": date.today().isoformat(), "base_url": BASE},
    ...
    if verbose:
        n_beta = sum(len(v) for v in betas.values())
        print(f"Scritto {out_path}: {n_beta} beta ({len(betas)} regioni), {len(erp)} ERP.")
    return dataset


def build_from_local(beta_files, erp_file=None, out_path="damodaran_dataset.json",
                     erp_extra=None, source=None, verbose=True):
    """Costruisce il dataset da file .xls LOCALI (se la rete è bloccata anche da te).
      beta_files : dict {regione: path_al_file_beta}
      erp_file   : path a ctryprem.xls (opzionale)
      erp_extra  : dict {paese: erp_pct} da unire ai valori noti (non sovrascrive il file)
    """
    betas = {}
    for region, path in beta_files.items():
        try:
            betas[region] = parse_beta_grid(read_grid(path, "Industry Name"))
            if verbose:
                print(f"  {region:9}: {len(betas[region])} settori  <- {path}")
        except Exception as e:
            if verbose:
                print(f"  {region:9}: ERRORE ({e})")
    erp = {}
    if erp_file:
        try:
            erp = parse_erp_grid(read_grid(erp_file, "Country"))
            if verbose:
                print(f"  ERP      : {len(erp)} paesi  <- {erp_file}")
        except Exception as e:
            if verbose:
                print(f"  ERP      : ERRORE ({e})")
    for k, v in (erp_extra or {}).items():
        erp.setdefault(k, v)
    return _write_dataset(betas, erp, source or "Aswath Damodaran (file locali)", out_path, verbose)


if __name__ == "__main__":
    # Modalità LOCALE se passi argomenti tipo:  Europe=betaEurope.xls  erp=ctryprem.xls
    # altrimenti scarica tutto dalla rete.
    args = [a for a in sys.argv[1:] if "=" in a]
    if args:
        beta_files, erp_file = {}, None
        for a in args:
            k, path = a.split("=", 1)
            if k.strip().lower() == "erp":
                erp_file = path
            else:
                beta_files[k.strip()] = path
        print("Costruzione dataset da file locali...")
        build_from_local(beta_files, erp_file=erp_file)
    else:
        print("Aggiornamento dati Damodaran (download)...")
        try:
            build_dataset()
        except Exception as e:
            ...
            print("Verifica connessione a pages.stern.nyu.edu e che 'xlrd' sia installato "
                  "(pip install xlrd). In alternativa scarica i file a mano e usa la modalità "
                  "locale:  python damodaran_update.py Europe=betaEurope.xls", file=sys.stderr)
            sys.exit(1)
