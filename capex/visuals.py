...
def plot_npv_distribution(npv_array, expected_npv, percentile_5, name):
    npv_array = np.array(npv_array, dtype=float)
    ...
    ax.axvline(0, color="black", linestyle=":", linewidth=1, label="Break-even (NPV=0)")
...
def plot_cumulative_npv(npv_cum_matrix, proj_name):
    npv_cum_matrix = np.array(npv_cum_matrix)
    ...
    # Payback sulla mediana: primo anno in cui NPV cumulato >= 0
    idx = np.where(median >= 0)[0]
    if idx.size > 0:
        pbp_median = int(idx[0]) + 1
...
def plot_payback_distribution(payback_array, project_name):
    payback_array = np.array(payback_array, dtype=float)

    fig, ax = plt.subplots(figsize=(10, 5))

    # solo valori finiti (no NaN / inf)
    ...
    ax.hist(pb, bins=range(1, max_bin + 2), alpha=0.75, edgecolor="black", color="#3b82f6")
    ax.set_title(f"Distribuzione Payback Period - {project_name}")
    ax.set_xlabel("Anni")
    ax.set_ylabel("Frequenza")
...
def plot_probs_kri(downside_prob, project_name):
    ...
    fig = go.Figure(go.Indicator(
        ...
        title={"text": f"KRI - {project_name}: {risk_level}", "font": {"size": 22, "color": "darkblue"}},
        gauge={
            ...
            "threshold": {"line": {"color": color, "width": 6}, "thickness": 0.8, "value": downside_prob * 100},
        }
    ))

    fig.update_layout(paper_bgcolor="white", font={"color": "darkblue", "family": "Arial"}, height=450)
    return fig


def plot_car_kri(car_value, expected_npv, project_name):
    """
    Gauge: rapporto CaR / Expected NPV (più alto => più rischioso)
    """
    soglia_alta = 0.50
    soglia_media = 0.25

    car_pct = (car_value / expected_npv) if expected_npv != 0 else 1.0
    car_pct = float(np.clip(car_pct, 0, 1))
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            "threshold": {"line": {"color": color, "width": 6}, "thickness": 0.8, "value": car_pct * 100},
        }
    ))
...
def plot_ppi_distribution(ppi_min, ppi_p5, ppi_p50, ppi_p95, ppi_max, years_labels=None,
    ...
    ppi_min = np.array(ppi_min)
    ppi_p5 = np.array(ppi_p5)
    ppi_p50 = np.array(ppi_p50)
    ppi_p95 = np.array(ppi_p95)
    ppi_max = np.array(ppi_max)
